...
def main():
    ...
    for p in data.get("prompts", []):
        pid=p.get("id")
        if not pid: continue
        search=p.get("search_text") or normalize(" ".join(str(p.get(k) or "") for k in ("title","title_ar","category","subcategory","prompt","prompt_ar","tags","models","model")))
        tokens=sorted(set(re.findall(r"[\w\u0600-\u06FF][\w\u0600-\u06FF+.#-]{1,}",search)))
        records.append({"id":pid,"title":p.get("title_ar") or p.get("title"),"title_en":p.get("title"),"task_type":p.get("task_type"),"modality":p.get("modality"),"category":p.get("category"),"subcategory":p.get("subcategory") or (p.get("subcategories") or []),"models":p.get("models") or ([p.get("model")] if p.get("model") else []),"tags":p.get("tags",[]),"quality_score":p.get("quality_score"),"prompt_hash":p.get("prompt_hash") or hashlib.sha256(normalize(p.get("prompt")).encode()).hexdigest(),"tokens":tokens})
        for token in tokens: inverted.setdefault(token,[]).append(pid)
    INDEX.write_text(json.dumps({"version":"1.2","generated_at":datetime.now(timezone.utc).isoformat(),"prompt_count":len(records),"records":records,"inverted_index":inverted},ensure_ascii=False,separators=(",",":"))+"\n",encoding="utf-8")
